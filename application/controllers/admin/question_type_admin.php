...
class Question_type_admin extends CI_Controller 
{
	...
	function add() {
		...
		{
			$this->form_validation->set_rules('name', 'Loại Câu Hỏi', 'trim|required|is_unique[question_type.name]');
	...
	function update($id=0) {
		if($this->input->post('submit'))
		{
			...
			$this->form_validation->set_rules('name', 'Loại Câu Hỏi', 'trim|required|callback_check_question_type_name');
			$this->form_validation->set_rules('description', 'Mô Tả', 'trim|required');
	...
	function check_question_type_name($name) {
		...
		{
			$this->form_validation->set_message('check_question_type_name', 'Loại Câu Hỏi này đã tồn tại.');
			return FALSE;
		}
		else
		{
