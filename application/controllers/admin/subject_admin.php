...
class Subject_admin extends CI_Controller {
	...
	function add() {
		...
		{
			$this->form_validation->set_rules('name', 'Tên Môn Học', 'trim|required|is_unique[subject.name]');
	...
	function update($id=0) {
		if($this->input->post('submit'))
		{
			...
			$this->form_validation->set_rules('name', 'Tên Môn Học', 'trim|required|callback_check_subject_name');
			$this->form_validation->set_rules('description', 'Mô Tả', 'trim|required');
	...
	function check_subject_name($name) {
		...
		{
			$this->form_validation->set_message('check_subject_name', 'Môn học này đã tồn tại');
			return FALSE;
		}
		else
		{
